...
def find_latest_model_by_architecture(base_dir, target_architecture=None):
    ...
    for search_dir in search_dirs:
        ...
        for ext in [".weights.h5", ".keras", ".h5"]:  # .weights.h5 primero
            files = [
                os.path.join(search_dir, item) for item in items if item.endswith(ext)
            ]
            ...
            for file_path in files:
                try:
                    ...
                    min_size = (
                        5 * 1024 * 1024
                        if file_path.endswith(".weights.h5")
                        else 1024 * 1024
                    )
            ...
            for item in items:
                ...
                if (
                    ...
                    not in [
                        ...
                        "vgg16",
                        "efficientnet",
                    ]  # Excluir carpetas de arquitectura
                    and os.path.exists(os.path.join(item_path, "saved_model.pb"))
                ):  # Validar SavedModel
...
def detect_model_architecture(model_path_or_layers):
    ...
    if isinstance(model_path_or_layers, str):
        ...
        vgg16_matches = sum(
            1
            for pattern in vgg16_patterns
            if any(pattern in name for name in layer_names)
        )
        if vgg16_matches >= 5:
            return "vgg16"

        # Detectar VGG16 - método 3: presencia de GlobalAveragePooling2D + estructura típica VGG16
        has_global_avg_pool = any(
            "global_average_pooling2d" in name for name in layer_names
        )
...
def build_model_by_architecture(architecture):
    ...
    if arch == "efficientnet":
        model, _ = (
            build_efficientnet_model()
        )  # Desempaquetar tupla, solo devolver modelo
        return model
    elif arch == "vgg16":
...
def load_model_by_architecture(
    architecture: str, stage="Production", compile_with_metrics=True
) -> keras.Model:
    """
    ...
    if MODEL_TARGET == "local":
        ...
        model_path = find_latest_model_by_architecture(
            local_model_directory, architecture
        )

        if not model_path:
            print(
                Fore.YELLOW
                + f"⚠️  No se encontró modelo de arquitectura {architecture}, usando carga general"
                + Style.RESET_ALL
            )
...
def load_model(stage="Production", compile_with_metrics=True) -> keras.Model:
    ...
    def reconstruct_model_from_weights(model_path, architecture):
        ...
        def try_load_weights(model, path, strategy_name):
            try:
                ...
                print(
                    Fore.GREEN
                    + f"   ✅ Pesos cargados exitosamente ({strategy_name})"
                    + Style.RESET_ALL
                )
                return True
            ...
            try:
                ...
                if architecture == "efficientnet":
                    ...
                    if strategy_type == "finetuning":
                        ...
                        print(
                            f"   🔧 Fine-tuning configurado: {fine_tune_at} capas congeladas"
                        )
        ...
        raise ValueError(
            f"No se pudo reconstruir el modelo {architecture} con ninguna estrategia"
        )
    ...
    def load_from_local():
        print(
            Fore.MAGENTA
            + "\n📥 Cargando modelo desde almacenamiento local..."
            + Style.RESET_ALL
        )
        local_model_directory = os.path.join(LOCAL_REGISTRY_PATH, "models")
        ...
        if not os.path.exists(local_model_directory):
            print(
                Fore.RED
                + f"❌ Directorio no existe: {local_model_directory}"
                + Style.RESET_ALL
            )
            return None

        # Buscar el modelo más reciente (puede estar en subcarpetas de arquitectura)
        most_recent_model_path_on_disk = find_latest_model_by_architecture(
        ...
        if not most_recent_model_path_on_disk:
            print(
                Fore.RED
                + "❌ No se encontraron modelos en el almacenamiento local."
                + Style.RESET_ALL
            )
            return None
        ...
        try:
            ...
            if most_recent_model_path_on_disk.endswith(".weights.h5"):
                print(
                    Fore.BLUE
                    + "🔧 Detectado archivo de pesos, reconstruyendo modelo..."
                    + Style.RESET_ALL
                )

                # Leer la configuración (buscar en el mismo directorio que el archivo de pesos)
                timestamp = os.path.basename(most_recent_model_path_on_disk).replace(
                    ".weights.h5", ""
                )
                ...
                if "architecture" in model_info:
                    ...
                    architecture = detect_model_architecture(
                ...
                model = reconstruct_model_from_weights(
                    most_recent_model_path_on_disk, architecture
                )
                ...
                try:
                    ...
                    if "No model config found" in str(e):
                        ...
                        print(
                            Fore.YELLOW
                            + "⚠️  Archivo contiene solo pesos, reconstruyendo modelo..."
                            + Style.RESET_ALL
                        )
            ...
            if compile_with_metrics:
                print(
                    Fore.BLUE
                    + "🔧 Recompilando con métricas completas..."
                    + Style.RESET_ALL
                )
                model.compile(
                    optimizer=keras.optimizers.Adam(learning_rate=0.001),
            ...
            display_name = arch_display_names.get(
    ...
    if MODEL_TARGET == "local":
        ...
        print(
            Fore.BLUE
            + "\n☁️  Cargando modelo desde Google Cloud Storage..."
            + Style.RESET_ALL
        )
        ...
        try:
                            ...
                            if not blob.name.endswith("/"):
                                if blob.name.endswith((".keras", ".h5", ".weights.h5")):
                                    if not blob.name.endswith("_config.json"):
                                        min_size = (
                                            5 * 1024 * 1024
                                            if blob.name.endswith(".weights.h5")
                                            else 1024 * 1024
                                        )
                                        if blob.size and blob.size > min_size:
                                            all_blobs.append(blob)
                                            print(
                                                f"   📦 Encontrado (versionado): {blob.name} ({blob.size / (1024*1024):.1f}MB)"
                                            )
                else:
                        ...
                        if not blob.name.endswith("/"):
                            ...
                            if blob.name.endswith((".keras", ".h5", ".weights.h5")):
                                ...
                                if not blob.name.endswith("_config.json"):
                                    ...
                                    )  # 5MB para weights, 1MB para otros
                                    if blob.size and blob.size > min_size:
                                        # Para modelos en directorio base, solo archivos directos
                                        if (
                                            prefix != "models/"
                                            or "/" not in blob.name[7:]
                                        ):
                                            all_blobs.append(blob)
                                            print(
                                                f"   📦 Encontrado: {blob.name} ({blob.size / (1024*1024):.1f}MB)"
                                            )
                                    else:
                                        print(
                                            f"   ⚠️  Archivo muy pequeño (posiblemente corrupto): {blob.name} ({blob.size / (1024*1024):.1f}MB)"
                                        )
            ...
            if not all_blobs:
                print(
                    f"{Fore.RED}❌ No se encontraron modelos válidos en GCS bucket {BUCKET_NAME}{Style.RESET_ALL}"
                )
                print(
                    f"{Fore.YELLOW}🔄 Intentando fallback a almacenamiento local...{Style.RESET_ALL}"
                )
                return load_from_local()
            ...
            if preferred_blobs:
                print(
                    f"🎯 Encontrados {len(preferred_blobs)} modelos de arquitectura preferida ({preferred_arch})"
                )
                ...
                if weights_blobs:
                    ...
                    print(
                        f"✅ Usando .weights.h5 de {preferred_arch}: {latest_blob.name}"
                    )
                else:
                ...
                print(
                    f"⚠️  No se encontraron modelos válidos de {preferred_arch}, usando el más reciente disponible"
                )
                # Fallback: usar cualquier modelo válido
                weights_blobs = [
                    blob for blob in all_blobs if blob.name.endswith(".weights.h5")
                ]
                if weights_blobs:
                    latest_blob = max(weights_blobs, key=lambda x: x.updated)
            ...
            if len(blob_path_parts) >= 2 and blob_path_parts[0] == "models":
                ...
                if potential_arch in ["cnn", "vgg16", "efficientnet"]:
                    ...
                    print(
                        f"🔍 Arquitectura detectada desde GCS: {detected_architecture}"
                    )

            # Crear directorio local manteniendo estructura de arquitectura
            if detected_architecture:
                local_model_directory = os.path.join(
                    LOCAL_REGISTRY_PATH, "models", detected_architecture
                )
            else:
                local_model_directory = os.path.join(LOCAL_REGISTRY_PATH, "models")
            ...
            latest_model_path_to_save = os.path.join(
                local_model_directory, model_filename
            )
            ...
            if latest_blob.name.endswith(".weights.h5"):
                config_blob_name = latest_blob.name.replace(
                    ".weights.h5", "_config.json"
                )
                ...
                try:
                    ...
                    if config_blob.exists():
                        ...
                        config_path = os.path.join(
                            local_model_directory, config_filename
                        )
                        config_blob.download_to_filename(config_path)
                        print(f"📥 Configuración descargada: {config_path}")
                    else:
                        print(
                            f"{Fore.YELLOW}⚠️  Archivo de configuración no encontrado: {config_blob_name}{Style.RESET_ALL}"
                        )
                except Exception as e:
                    print(
                        f"{Fore.YELLOW}⚠️  Error descargando configuración: {e}{Style.RESET_ALL}"
                    )

            # Ahora cargar el modelo usando la misma lógica que load_from_local
            try:
                print("🔄 Cargando modelo descargado...")
                ...
                if latest_model_path_to_save.endswith(".weights.h5"):
                    ...
                    if config_path and os.path.exists(config_path):
                        ...
                        useefficientnet = (
                            "EfficientNet" in model_filename
                            or "efficientnet" in model_filename.lower()
                        )
                        print(
                            f"{Fore.YELLOW}⚠️  No hay configuración, detectando por nombre: {'EfficientNet' if useefficientnet else 'CNN'}{Style.RESET_ALL}"
                        )
                        ...
                        try:
                            ...
                            if useefficientnet:
                                if strategy_type == "finetuning":
                                    ...
                                    print(
                                        f"🔧 Fine-tuning configurado: {fine_tune_at} capas congeladas, {15} entrenables"
                                    )
                            ...
                            # Intentar cargar pesos
                            print(
                                f"📊 Modelo reconstruido tiene {len(model.layers)} capas"
                            )
                            ...
                            try:
                                ...
                                with h5py.File(latest_model_path_to_save, "r") as f:
                                    print(
                                        f"📦 Archivo de pesos contiene información para {len(f.keys())} grupos"
                                    )
                            except Exception:
                                print("📦 Analizando archivo de pesos...")
                            ...
                            if "Layer count mismatch" in error_msg:
                                print(
                                    f"❌ {strategy_name}: Incompatibilidad de capas - {error_msg}"
                                )
                            elif "axes don't match array" in error_msg:
                                print(
                                    f"❌ {strategy_name}: Incompatibilidad de dimensiones - {error_msg}"
                                )
                            else:
                    ...
                    if not model_loaded:
                        print(
                            Fore.RED
                            + "❌ No se pudo reconstruir el modelo con ninguna estrategia"
                            + Style.RESET_ALL
                        )
                        print(
                            Fore.YELLOW
                        ...
                        raise ValueError(
                            "No se pudo reconstruir el modelo compatible con los pesos guardados"
                        )
                    ...
                    try:
                        ...
                        if "No model config found" in str(e):
                            ...
                            print(
                                f"🔍 Detectado tipo: {'EfficientNet' if useefficientnet else 'CNN simple'} basado en nombre: {model_filename}"
                            )
                            ...
                            for (
                                strategy_name,
                                strategy_type,
                            ) in reconstruction_strategies:
                                try:
                                    print(
                                        f"🔍 Intentando estrategia: {strategy_name}..."
                                    )
                                    ...
                                    if "Layer count mismatch" in error_msg:
                                        ...
                                        print(
                                            f"❌ {strategy_name}: Error - {error_msg}"
                                        )
                                    continue
                            ...
                            if not model_loaded:
                                ...
                                print(
                                    f"   🔍 Tipo detectado: {'EfficientNet' if useefficientnet else 'CNN'}"
                                )
                                raise ValueError(
                                    "No se pudo reconstruir el modelo compatible con los pesos guardados"
                                )
                ...
                print(
                    Fore.GREEN
                    + "✅ Modelo cargado exitosamente desde GCS"
                    + Style.RESET_ALL
                )
                ...
                print(
                    Fore.RED
                    + f"❌ Error al cargar modelo descargado: {e}"
                    + Style.RESET_ALL
                )
                import traceback
        ...
        print(
            Fore.MAGENTA
            + f"\n📥 Cargando modelo desde MLflow (stage: {stage})..."
            + Style.RESET_ALL
        )
        ...
        try:
            ...
            if not model_versions:
                print(
                    Fore.YELLOW
                    + f"⚠️  No se encontró modelo en stage '{stage}'"
                    + Style.RESET_ALL
                )
                print(
                    Fore.BLUE
                    + "🔍 Intentando cargar la última versión disponible..."
                    + Style.RESET_ALL
                )
                ...
                if not all_versions:
                    print(
                        Fore.RED
                        + f"❌ No se encontró ningún modelo '{MLFLOW_MODEL_NAME}' en MLflow"
                        + Style.RESET_ALL
                    )
                    print(
                        Fore.YELLOW
                        + "🔄 Intentando fallback a almacenamiento local..."
                        + Style.RESET_ALL
                    )
                    return load_from_local()
            ...
            if storage_format == "weights_only":
                print(
                    Fore.BLUE
                    + "🔧 Detectado formato de pesos, reconstruyendo modelo..."
                    + Style.RESET_ALL
                )
                ...
                with tempfile.TemporaryDirectory() as tmp_dir:
                    ...
                    print(
                        Fore.GREEN + "✅ Pesos cargados exitosamente" + Style.RESET_ALL
                    )
            else:
                ...
                try:
                    ...
                    with tempfile.TemporaryDirectory() as tmp_dir:
                        local_model_path = mlflow.artifacts.download_artifacts(
                            artifact_uri=f"{model_uri}/model", dst_path=tmp_dir
                        )
            ...
            print(
                Fore.RED
                + f"❌ Error al cargar modelo desde MLflow: {e}"
                + Style.RESET_ALL
            )
            print(
                Fore.YELLOW
        ...
        print(
            Fore.RED + f"❌ MODEL_TARGET no válido: '{MODEL_TARGET}'" + Style.RESET_ALL
        )
        print(
            Fore.YELLOW + "💡 Valores permitidos: 'local' o 'mlflow'" + Style.RESET_ALL
        )
        return None
...
def save_results(params: dict, metrics: dict):
    ...
    if active_run or MODEL_TARGET == "mlflow":
        ...
        print(
            Fore.YELLOW
            + "⚠️  No active MLflow run and MODEL_TARGET != 'mlflow'"
            + Style.RESET_ALL
        )
    ...
    print(
        Fore.GREEN
        + f"\n✅ Resultados guardados localmente ({'y MLflow' if mlflow_logged else 'solamente'})."
        + Style.RESET_ALL
    )
...
def save_model(model: keras.Model = None) -> None:
    ...
    arch_map = {"efficientnet": "EfficientNet", "vgg16": "VGG16", "cnn": "CNN"}
    ...
    try:
        ...
        print(
            Fore.GREEN
            + f"\n✅ Modelo guardado localmente en: {model_path}"
            + Style.RESET_ALL
        )
        ...
        if "Unable to serialize" in str(e) or "EagerTensor" in str(e):
            ...
            print(
                Fore.BLUE
                + "🔄 Guardando en formato de pesos separados..."
                + Style.RESET_ALL
            )
            ...
            model_info = {
                ...
                "output_shape": (
                    list(model.output_shape) if model.output_shape else None
                ),
            ...
            print(
                Fore.GREEN
                + "\n✅ Modelo guardado en formato de pesos:"
                + Style.RESET_ALL
            )
            ...
            print(
                Fore.BLUE
                + "ℹ️  Para cargar: usar load_model() que reconstruirá el modelo"
                + Style.RESET_ALL
            )
        else:
            raise e
    ...
    if MODEL_TARGET == "gcs":
        ...
        if saved_file_path and os.path.exists(saved_file_path):
            ...
            if saved_file_path.endswith(".weights.h5"):
                ...
                config_path = os.path.join(
                    os.path.dirname(saved_file_path), config_filename
                )
            ...
            print(
                f"❌ Error: No se pudo subir a GCS, archivo no válido: {saved_file_path}"
            )
    ...
    if MODEL_TARGET == "mlflow":
        ...
        save_model_to_mlflow(
            model=model, params=None, metrics=None, architecture=architecture
        )
...
def mlflow_transition_model(current_stage: str, new_stage: str) -> None:
    ...
    print(
        f"✅ El modelo {MLFLOW_MODEL_NAME} ha sido movido de {current_stage} a {new_stage}."
    )
...
def save_model_to_mlflow(model, params, metrics, architecture):
    ...
    try:
        ...
        print(
            Fore.YELLOW
            + f"⚠️  Modelo '{MLFLOW_MODEL_NAME}' no existe, creándolo..."
            + Style.RESET_ALL
        )
        try:
            ...
            print(
                Fore.GREEN
                + f"✅ Modelo registrado '{MLFLOW_MODEL_NAME}' creado exitosamente"
                + Style.RESET_ALL
            )
        except Exception as e:
            print(
                Fore.RED + f"❌ Error al crear modelo registrado: {e}" + Style.RESET_ALL
            )
    ...
    try:
        with mlflow.start_run() as run:
            ...
            with tempfile.NamedTemporaryFile(suffix=".keras", delete=False) as tmp:
                try:
                    ...
                    if "Unable to serialize" in str(e) or "EagerTensor" in str(e):
                        print(
                            Fore.YELLOW
                            + "⚠️  No se puede serializar modelo completo, usando formato de pesos..."
                            + Style.RESET_ALL
                        )
                        ...
                        model_info = {
                            "input_shape": (
                                list(model.input_shape) if model.input_shape else None
                            ),
                            "output_shape": (
                                list(model.output_shape) if model.output_shape else None
                            ),
                        ...
                        model_size_mb = os.path.getsize(tmp_weights_path) / (
                            1024 * 1024
                        )
            ...
            if model_size_mb < 50:  # Umbral de 50MB
                ...
                print(
                    Fore.YELLOW
                    + f"⚠️  Modelo muy grande ({model_size_mb:.1f}MB)"
                    + Style.RESET_ALL
                )
        ...
        print(
            Fore.YELLOW
            + f"💾 Guardando localmente como fallback... ({str(e)[:50]})"
            + Style.RESET_ALL
        )
        ...
        try:
            ...
            try:
                ...
                print(
                    Fore.BLUE
                    + "🔄 Guardando solo pesos localmente..."
                    + Style.RESET_ALL
                )
            ...
            print(
                Fore.GREEN
                + f"✅ Modelo guardado localmente: {local_path}"
                + Style.RESET_ALL
            )
            ...
            print(
                Fore.RED + f"❌ Error en fallback: {fallback_error}" + Style.RESET_ALL
            )
...
def load_specific_model(model_path: str) -> keras.Model:
    ...
    try:
        print(
            Fore.BLUE
            + f"📁 Cargando modelo específico: {os.path.basename(model_path)}"
            + Style.RESET_ALL
        )
        ...
        try:
            model = keras.models.load_model(
                model_path, custom_objects={"DiseaseRecallMetric": DiseaseRecallMetric}
            )

            print(
                Fore.GREEN + "✅ Modelo completo cargado exitosamente" + Style.RESET_ALL
            )

        except Exception as e:
            print(
                Fore.YELLOW
                + f"⚠️  No se pudo cargar como modelo completo: {str(e)[:100]}"
                + Style.RESET_ALL
            )
            print(
                Fore.YELLOW
                + "🔄 Intentando carga mediante load_model() genérico..."
                + Style.RESET_ALL
            )
        ...
        print(
            Fore.GREEN + "✅ Modelo específico cargado exitosamente" + Style.RESET_ALL
        )
        return model
